...
export const SchoolAdminLayout: React.FC = () => {
  ...
  // Role-based access control for school admin layout
  useEffect(() => {
    if (!loading) {
      if (!user || !profile) {
        console.log('SchoolAdminLayout: No user or profile, redirecting to login');
        navigate('/login');
        return;
      }

      if (profile.role !== 'school_admin') {
        console.log('SchoolAdminLayout: User is not school_admin, redirecting based on role');
        switch (profile.role) {
          case 'platform_admin':
      ...
      if (!profile.approved) {
        console.log('SchoolAdminLayout: User not approved');
        navigate('/login');
        return;
      }
    }
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="flex items-center justify-between h-16 px-4 border-b border-gray-200">
            <div className="flex items-center">
              ...
              <span className="text-lg font-semibold text-gray-900">School Admin</span>
            </div>
            <button
              onClick={() => setSidebarOpen(false)}
          ...
          <div className="flex items-center justify-between h-16 px-4 sm:px-6 lg:px-8">
            <div className="flex items-center">
              ...
              <h1 className="ml-2 lg:ml-0 text-xl font-semibold text-gray-900">
                SchoolConnect - School Dashboard
              </h1>
            </div>
