-- Complete SchoolConnect Database Schema
...
CREATE POLICY "School admins can view school users" ON public.users
    FOR SELECT USING (
        auth.uid() IN (
            SELECT id FROM public.users 
            WHERE role = 'school_admin' AND school_id = (SELECT school_id FROM public.users WHERE id = auth.uid())
        )
    );
...
CREATE POLICY "Platform admins can view all schools" ON public.schools
    FOR SELECT USING (
        auth.uid() IN (SELECT id FROM public.users WHERE role = 'platform_admin')
    );
